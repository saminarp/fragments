...
async function deleteFragment(ownerId, id) {
  // api parameters for deleting operation
  const params = {
    TableName: process.env.AWS_DYNAMODB_TABLE_NAME,
    Key: {
      ownerId,
  ...
  const deleteddbCommand = new DeleteCommand(params);
  ...
  const deleteS3Command = new DeleteObjectCommand(params2);

  try {
    await s3Client.send(deleteS3Command);
  } catch (err) {
    const { Bucket, Key } = params2;
    logger.error({ err, Bucket, Key }, 'Error deleting fragment from S3 Bucket');
    throw new Error('unable to delete fragment from S3');
  }
}
